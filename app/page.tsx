    ...
    <main className="game-shell">
      ...
      <section className="scoreboard">
        {(["a", "b"] as Team[]).map((team) => (
          <div className={`score-card ${team === "a" ? "sun" : "moon"}`} key={team}>
...
function Lobby({ game, teams, isHost, durationSeconds, roundLimit, setTeams, setDurationSeconds, setRoundLimit, assign, start, busy }: { game: GameState; teams: Record<string, Team>; isHost: boolean; durationSeconds: number; roundLimit: number; setTeams: (teams: Record<string, Team>) => void; setDurationSeconds: (value: number) => void; setRoundLimit: (value: number) => void; assign: () => void; start: () => void; busy: boolean }) {
  ...
  return <section className="panel lobby"><p className="eyebrow">THE LOBBY</p><h2>Gather your crew</h2><p className="muted">Share code <strong>{game.room.code}</strong>. The host makes two teams of two.</p>
    <div className="players">{game.players.map((player) => <div className="player-row" key={player.id}><span className="avatar">{player.displayName.slice(0, 1)}</span><strong>{player.displayName}</strong>{isHost ? <select value={teams[player.id] ?? "a"} onChange={(event) => setTeams({ ...teams, [player.id]: event.target.value as Team })}><option value="a">Team Sun</option><option value="b">Team Moon</option></select> : <span className="team-pill">{player.team ? teamName(player.team) : "Waiting for host"}</span>}</div>)}</div>
...
function RevealPhase({ game, isHost, next, busy }: { game: GameState; isHost: boolean; next: () => void; busy: boolean }) {
  const winner = game.room.phase === "finished" ? (game.room.scoreA === game.room.scoreB ? "It’s a tie!" : game.room.scoreA > game.room.scoreB ? "Team Sun wins!" : "Team Moon wins!") : "Round results";
